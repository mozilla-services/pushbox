...
impl Metrics {
    ...
    pub fn init(config: &Config) -> error::Result<Metrics> {
        let logging = logging::RBLogger::new(config);
        let builder = match config.get_string("statsd_host") {
            Ok(statsd_host) => {
                let socket = UdpSocket::bind("0.0.0.0:0")
                    .map_err(|e| error::HandlerErrorKind::GeneralError(format!("{:?}", e)))?;
                socket
                ...
                StatsdClient::builder(
                    &config
                        ...
                        .unwrap_or_else(|_| "megaphone".to_string()),
                    sink,
                )
            }
